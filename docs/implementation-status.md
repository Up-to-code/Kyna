...
| Cross-platform distribution | zero-clone macOS/Linux/Windows archives, per-user shell/PowerShell installers, canonical `@kyna-language/cli` npm installer plus its `kyna` short alias, self-management, GHCR image, Dev Container, signing/notarization gates, checksums, and provenance workflows implemented; first registry publication and clean hosted-runner release validation remain release gates; other package-manager manifests and multi-version toolchain management are deferred |
